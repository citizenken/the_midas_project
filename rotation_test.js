...
window.onload = (function() {
    ...
    /*--- initialize Crafty ---*/
    Crafty.init(WIDTH, HEIGHT); /*--- load images ---*/
...
/*--- component drone ---------------------------------
    ...
    Crafty.c("f_drone", {
        ...
        checkRotation: function() {
            if (Math.abs(this.rotation) > 360) this.rotation %= this.rotation;
        },
        turnLeft: function() {
            this.checkRotation();
            this.rotation -= this._turnSpeed;
        },
...
/*--- component Playground ----------------------------
    ...
    Crafty.c("Playground", {
        init: function() {
            var tmp_drone = Crafty.e("2D, Canvas, Mouse, f_drone").attr({
                x: 100,
                y: 100
            });
        ...
        // update every frame
        this.bind("EnterFrame", function() {
            var tmp_x = Crafty.mousePos.x;
            var tmp_y = Crafty.mousePos.y;
            var tmp_origin_x = tmp_drone._origin.x;
            var tmp_origin_y = tmp_drone._origin.y;
            var tmpD = Math.atan2(tmp_y - tmp_drone.y + tmp_origin_y, tmp_x - tmp_drone.x + tmp_origin_x) * (180 / Math.PI) + 90;

            var tD = tmp_drone.rotation % 360 - tmpD;

            if (tD > 0) if (Math.abs(tD) > 180) tmp_drone.turnRight()
            else tmp_drone.turnLeft();

            if (tD < 0) if (Math.abs(tD) > 180) tmp_drone.turnLeft();
            else tmp_drone.turnRight();
        });
    }
}); /*--- end component Playground ---*/

/*--- scene loading -----------------------------------
    -------------------------------------------------------
    -----------------------------------------------------*/
Crafty.scene("loading", function() {
    Crafty.load(["http://keinpunkt.de/warleague/drone_16x7.png"], function() { /*--- play scene playing ---*/
