...
Crafty.extend({
    ...
    mouseDispatch: function (e) {
        ...
        if (closest) {
            ...
            if (type === "mousedown") {
                ...
                if (this.over !== closest) { //if new mousemove, it is over
                    if (this.over) {
                        this.over.trigger("MouseOutTP2", e); //if over wasn't null, send mouseout
                        this.over = null;
                    }
                    this.over = closest;
    ...
    touchDispatch: function (e) {
        var type,
            type2,
            lastEvent = Crafty.lastTouchEvent;
        ...
        //Determine which touch changed
        var changedTouches = e.gesture.srcEvent.changedTouches;
        var touches = e.gesture.srcEvent.touches;

        //Assign touches to correct Crafty.touches position if they are not already set
        if (touches.length === 1) {
        ...
        //Determine if the first or second touch is changing
        ...
        //if it's a DOM element with Mouse component we are done
        // console.log('node name', tar.nodeName);
        if (tar.nodeName != "CANVAS") {
            while (typeof (tar.id) != 'string' && tar.id.indexOf('ent') == -1) {
                tar = tar.parentNode;
            }
        ...
        if (first && second) {
            ...
            if (!closest2) {
                ...
                // console.log('seconds qlength', q, q.length)
                for (l = q.length; g < l; ++g) {
                    ...
                    var current = q[g],
                        flag = false;
                    //weed out duplicates
                    if (dupes2[current[0]]) continue;
                    else dupes2[current[0]] = true;
