...
Crafty.c("Player", {
    ...
    _tradeItems: [],
    init: function() {
        this.requires("Actor, Twoway, Collision, WireBox,Delay, Persist, Solid, Color, Gravity").color("black").gravity("Floor").attr({
            h: 64
        }).gravityConst(.22).twoway(this._moveSpeed, this._jumpSpeed).bind("EnterFrame", function(data) {
            this.checkDead();
        }).bind("KeyDown", function(e) {
            switch (e.key) {
