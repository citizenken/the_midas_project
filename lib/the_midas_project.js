(function e(t, n, r) {
    ...
    8: [ function(require, module, exports) {
        ...
        Crafty.extend({
            ...
            touchDispatch: function(e) {
                var type, type2, lastEvent = Crafty.lastTouchEvent;
                if (!Crafty.touchObjs) return;
                Crafty.lastEvent = e;
                var maxz = -1, closest, closest2, q, i = 0, g = 0, l, pos = {}, pos2 = {}, x, y, x2, y2, dupes = {}, dupes2 = {}, tar = e.target ? e.target : e.srcElement, type = e.type, first, second;
                var changedTouches = e.gesture.srcEvent.changedTouches;
                var touches = e.gesture.srcEvent.touches;
                if (touches.length === 1) {
                    var curTouchIndex = Crafty.touches.indexOf(touches[0].identifier);
                    if (curTouchIndex < 0) {
                        Crafty.touches[0] = touches[0].identifier;
                    }
                } else if (touches.length === 2) {
                    var curTouchIndex1 = Crafty.touches.indexOf(touches[0].identifier);
                    var curTouchIndex2 = Crafty.touches.indexOf(touches[1].identifier);
                    if (curTouchIndex1 < 0) {
                        Crafty.touches[0] = touches[0].identifier;
                    } else if (curTouchIndex1 === 1) {
                        Crafty.touches[0] = touches[0].identifier;
                        Crafty.touches[1] = null;
                    }
                    if (curTouchIndex2 < 0) {
                        Crafty.touches[1] = touches[1].identifier;
                    } else if (curTouchIndex1 === 0) {
                        Crafty.touches[1] = touches[1].identifier;
                        Crafty.touches[0] = touches[0].identifier;
                    }
                }
                for (var a = 0; a < changedTouches.length; a++) {
                    var curTouchIndex = Crafty.touches.indexOf(changedTouches[a].identifier);
                    if (curTouchIndex === 0) {
                        first = changedTouches[a];
                        pos.x = changedTouches[a].clientX;
                        pos.y = changedTouches[a].clientY;
                    } else if (curTouchIndex === 1) {
                        second = e.gesture.touches[a];
                        pos2.x = changedTouches[a].clientX;
                        pos2.y = changedTouches[a].clientY;
                    }
                }
                if (first && second) {
                    e.realX = x = Crafty.touch1Pos.x = pos.x;
                    e.realY = y = Crafty.touch1Pos.y = pos.y;
                    e.realX2 = x2 = Crafty.touch2Pos.x = pos2.x;
                    e.realY2 = y2 = Crafty.touch2Pos.y = pos2.y;
                } else if (first) {
                    e.realX = x = Crafty.touch1Pos.x = pos.x;
                    e.realY = y = Crafty.touch1Pos.y = pos.y;
                } else if (second) {
                ...
                if (first && second) {
                    if (!closest) {
                        ...
                        for (l = q.length; i < l; ++i) {
                            if (!q[i].__c.Touch || !q[i]._visible) continue;
                            var current = q[i], flag = false;
                            if (dupes[current[0]]) continue; else dupes[current[0]] = true;
                            if (current.mapArea) {
                                if (current.mapArea.containsPoint(x, y)) {
                    ...
                    if (!closest2) {
                        ...
                        for (l = q.length; g < l; ++g) {
                            if (!q[g].__c.Touch || !q[g]._visible) continue;
                            var current = q[g], flag = false;
                            if (dupes2[current[0]]) continue; else dupes2[current[0]] = true;
                            if (current.mapArea) {
                                if (current.mapArea.containsPoint(x2, y2)) {
                                    flag = true;
                                }
                            } else if (current.isAt(x2, y2)) flag = true;
                            if (flag && (current._z >= maxz || maxz === -1)) {
                                if (current._z === maxz && current[0] < closest[0]) {
                                    continue;
                                }
                ...
                if (closest && closest2) {
                    if (type === "tap") {
                        closest.trigger("Tap1", e);
                        closest2.trigger("Tap1", e);
                    } else if (type === "touch") {
                        closest.trigger("Touch", e);
                        closest2.trigger("Touch2", e);
                    } else if (type == "release") {
                        closest.trigger("Release1", e);
                        closest2.trigger("Release2", e);
                    } else if (type == "doubletap") {
                        closest.trigger("DoubleTap1", e);
                        closest2.trigger("DoubleTap2", e);
                    } else if (type === "hold") {
                        closest.trigger("Hold1", e);
                        closest2.trigger("Hold2", e);
                    } else if (type === "swipe") {
                        closest.trigger("Swipe1", e);
                        closest2.trigger("Swipe2", e);
                    } else if (type === "drag") {
                        closest.trigger("TouchDrag1", e);
                        closest2.trigger("TouchDrag2", e);
                    } else if (type === "dragstart") {
                        closest.trigger("TouchDragStart1", e);
                        closest2.trigger("TouchDragStart2", e);
                    } else if (type === "dragend") {
                        closest.trigger("TouchDragEnd1", e);
                        closest2.trigger("TouchDragEnd2", e);
                    } else if (type == "transform") {
                        closest.trigger("Transform1", e);
                        closest2.trigger("Transform2", e);
                    } else if (type == "transformstart") {
                        closest.trigger("TransformStart1", e);
                        closest2.trigger("TransformStart2", e);
                    } else if (type == "transformend") {
                        closest.trigger("TransformEnd1", e);
                        closest2.trigger("TransformEnd2", e);
                    }
                } else if (closest) {
                    if (type === "tap") {
                        closest.trigger("Tap1", e);
                    } else if (type === "touch") {
                        closest.trigger("Touch1", e);
                    } else if (type == "release") {
                        closest.trigger("Release1", e);
                    } else if (type == "doubletap") {
                        closest.trigger("DoubleTap1", e);
                        ...
                        console.log("sending transform end 1");
                        closest.trigger("TransformEnd1", e);
                    }
                } else if (closest2) {
                    if (type === "tap") {
                        closest2.trigger("Tap2", e);
                    } else if (type === "touch") {
                        closest2.trigger("Touch2", e);
                    } else if (type == "release") {
                        closest2.trigger("Release2", e);
        ...
        Crafty.bind("Load", function() {
            ...
            Crafty.addEvent(this, Crafty.stage.elem, "tap", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "touch", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "release", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "doubletap", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "hold", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "swipe", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "drag", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "dragstart", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "dragend", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "doubletap", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "transformstart", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "transform", Crafty.touchDispatch, true);
            Crafty.addEvent(this, Crafty.stage.elem, "transformend", Crafty.touchDispatch, true);
        });
        Crafty.bind("CraftyStop", function() {
            ...
            if (Crafty.stage) {
                ...
                Crafty.removeEvent(this, Crafty.stage.elem, "tap", Crafty.touchDispatch, true);
                Crafty.removeEvent(this, Crafty.stage.elem, "touch", Crafty.touchDispatch, true);
                Crafty.removeEvent(this, Crafty.stage.elem, "release", Crafty.touchDispatch, true);
                Crafty.removeEvent(this, Crafty.stage.elem, "doubletap", Crafty.touchDispatch, true);
                Crafty.removeEvent(this, Crafty.stage.elem, "hold", Crafty.touchDispatch, true);
                Crafty.removeEvent(this, Crafty.stage.elem, "swipe", Crafty.touchDispatch, true);
                Crafty.removeEvent(this, Crafty.stage.elem, "drag", Crafty.touchDispatch, true);
                Crafty.removeEvent(this, Crafty.stage.elem, "doubletap", Crafty.touchDispatch, true);
            }
        ...
        Crafty.c("Draggable", {
            ...
            _ondrag: function(e) {
                if (Crafty.mobile) {
                    var pos = Crafty.DOM.translate(e.realX, e.realY);
                } else {
                ...
                if (this._dir) {
                    console.log("this dir is set");
                    if (Crafty.mobile) {
                        var len = (pos.x - this._origTouchDOMPos.x) * this._dir.x + (pos.y - this._origTouchDOMPos.y) * this._dir.y;
                    } else {
                    ...
                    console.log("this dir is not set");
                    if (Crafty.mobile) {
                        this.x = this._oldX + (pos.x - this._origTouchDOMPos.x);
                        this.y = this._oldY + (pos.y - this._origTouchDOMPos.y);
                        console.log("pos", pos.x, pos.y, "orig touch pos", this._origTouchDOMPos.x, this._origTouchDOMPos.y);
                        console.log("old x", this._oldX, "old x", this._oldY, "new x", this.x, "new y", this.y);
                    } else {
                        this.x = this._oldX + (pos.x - this._origMouseDOMPos.x);
                        this.y = this._oldY + (pos.y - this._origMouseDOMPos.y);
                        console.log("pos", pos.x, pos.y, "orig touch pos", this._origMouseDOMPos.x, this._origMouseDOMPos.y);
                        console.log("old x", this._oldX, "old x", this._oldY, "new x", this.x, "new y", this.y);
                    }
                }
            ...
            _onup: function(e) {
                if (this._dragging === true) {
                    if (Crafty.mobile) {
                        Crafty.removeEvent(this, Crafty.stage.elem, "drag", this._ondrag, true);
                        Crafty.removeEvent(this, Crafty.stage.elem, "release", this._onup, true);
                    } else {
            ...
            _startDrag: function(e) {
                if (Crafty.mobile) {
                    this._origTouchDOMPos = Crafty.DOM.translate(e.realX, e.realY);
                } else {
                ...
                this._dragging = true;
                if (Crafty.mobile) {
                    Crafty.addEvent(this, Crafty.stage.elem, "drag", this._ondrag, true);
                    Crafty.addEvent(this, Crafty.stage.elem, "release", this._onup, true);
                } else {
            ...
            enableDrag: function() {
                if (Crafty.mobile) {
                    this.bind("TouchDragStart1", this._ondown);
                    this.bind("TouchDragStart2", this._ondown);
                } else {
            ...
            disableDrag: function() {
                if (Crafty.mobile) {
                    this.bind("TouchDragEnd1", this._ondown);
                    this.bind("TouchDragEnd2", this._ondown);
                } else {
    ...
    13: [ function(require, module, exports) {
        ...
        Crafty.extend({
            _events: {},
            addEvent: function(ctx, obj, type, callback, hammer) {
                hammer = hammer || false;
...
Crafty.c("Camel", {
    ...
    init: function() {
        ...
        this._speed = Math.round(Crafty.math.randomNumber(.5, 2));
        this._oldMovementx;
        this._oldMovementy;
...
Crafty.c("MouseFace", {
    ...
    _onmousemove: function(e) {
        ...
        var dx = this.x - e.realX, dy = this.y - e.realY;
        if (this._origin) {
            dx += this._origin.x;
            dy += this._origin.y;
        }
        ...
        if (Crafty.math.withinRange(this._dirAngle, this._pi_4, this.pi_4)) {
            this._dirMove = this._directions.left;
        } else if (Crafty.math.withinRange(this._dirAngle, this.pi_4, this.pi_34)) {
            this._dirMove = this._directions.up;
        } else if (Crafty.math.withinRange(this._dirAngle, this.pi_34, this.pi)) {
            this._dirMove = this._directions.right;
        }
        if (Crafty.math.withinRange(this._dirAngle, this._pi, this._pi_34)) {
            this._dirMove = this._directions.right;
        } else if (Crafty.math.withinRange(this._dirAngle, this._pi_34, this._pi_4)) {
            this._dirMove = this._directions.down;
        } else if (Crafty.math.withinRange(this._dirAngle, this._pi_4, 0)) {
            this._dirMove = this._directions.left;
        }
    },
    init: function() {
...
Crafty.c("Enemy", {
    ...
    _maxSteps: 0,
    init: function() {
        this.requires("Actor, Fourway, Collision, SpriteAnimation, WiredHitBox, spr_blue_player, TargetMovement").attr({
            steps: 0,
            direction: null,
            aggroDistance: 200,
            targetLocation: {
        ...
        }).reel("EnemyUp", 400, 0, 0, 3).reel("EnemyDown", 400, 0, 2, 3).reel("EnemyRight", 400, 0, 1, 3).reel("EnemyLeft", 400, 0, 3, 3).bind("NewDirection", function(movement) {
            if (movement.x != this._oldMovementx || movement.y != this._oldMovementy) {
                if (movement.y == -1) {
...
Crafty.c("JoystickBase", {
    ...
    init: function() {
        ...
        this.h = 100;
        this.w = 100;
        this.areaMap([ this._x, this._y ], [ this._w, this._y ], [ this._x, this._h ], [ this._w, this._h ]);
...
Crafty.c("Arm", {
    ...
    init: function() {
        ...
        this.requires("2D, Canvas, Color, Collision, MouseFace");
        this.origin(2, 2);
        this.attr({
            move: {
                left: false,
            ...
            x: player._armLocation.x,
            y: player._armLocation.y,
            z: 2 + 1,
            moving: false,
            curAngle: 0
        });
...
Crafty.c("LeadCamel", {
    ...
    _hitPoints: 1,
    init: function() {
        this.requires("Actor, Fourway, Collision, SpriteAnimation, spr_lead_camel_white").fourway(2).bind("Moved", function(oldLocation) {
...
Crafty.c("Player", {
    ...
    checkAdjacent: function() {
        var currentSquare = {
            x: Math.floor(this.at().x),
            y: Math.floor(this.at().y)
        };
...
Crafty.c("Tree", {
    init: function() {
        this.requires("Scenery, Color").color("rgb(20, 125, 40)");
    }
});

Crafty.c("Gate", {
    init: function() {
        this.requires("Scenery, Color").color("black");
    }
});
...
Crafty.c("Floor", {
    ...
    _currentGold: 0,
    init: function() {
        this.requires("Actor, Solid, Collision, Color").color("blue");
    }
});
...
Crafty.c("Button", {
    ...
    _currentGold: 0,
    init: function() {
        this.requires("Scenery, Color, InterActive").color("red");
        this.h = 10;
        this.w = 10;
    }
});
...
Crafty.c("Mountain", {
    init: function() {
        this.requires("Scenery, Color").color("brown");
    }
});

Crafty.c("Mountain", {
    init: function() {
        this.requires("Scenery, Color, Touch").color("brown");
        this.h = 100;
        this.w = 100;
...
Crafty.c("Bush", {
    init: function() {
        this.requires("Scenery, Color").color("rgb(20, 185, 40)");
    }
});

Crafty.c("SandDune", {
    init: function() {
        this.requires("Actor, Color").color("white");
        this._z = -1;
    }
});
...
Crafty.c("TextBox", {
    ...
    getLocation: function() {
        ...
        this.attr({
            x: viewportX + 50 * dirmodx,
            y: viewportY + 50 * dirmody
        });
...
Crafty.scene("Loading", function() {
    ...
    Crafty.load([ "assets/actors/human.png" ], function() {
...
var labLvl1 = [ "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff", "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff", "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff", "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff", "######################################################################################################################################################################################", "######################################################################################################################################################################################", "######################################################################################################################################################################################", "######################################################################################################################################################################################", "######################################################################################################################################################################################", "######################################################################################################################################################################################", "######################################################################################################################################################################################", "#################b####################################################################################################################################################################", "###############ffffffff###ffffffffff##################################################################################################################################################", "#@#####b######fffffffff###ffffffffff##################################################################################################################################################", "############fffffffffff###ffffffffff##################################################################################################################################################", "fffffffffffffffffffffff###ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff", "fffffffffffffffffffffff###ffffffffff##################################################################################################################################################", "fffffffffffffffffffffff###ffffffffff##################################################################################################################################################" ];
