    ...
    8: [ function(require, module, exports) {
        ...
        Crafty.extend({
            ...
            touchDispatch: function(e) {
                ...
                Crafty.lastEvent = e;
                var maxz = -1, closest, q, i = 0, l, pos = Crafty.DOM.translate(e.realX, e.realY), x, y, dupes = {}, tar = e.target ? e.target : e.srcElement, type = e.type, first, second;
                for (var i = 0; i < e.gesture.touches.length; i++) {
                    var curTouchIndex = Crafty.touches.indexOf(e.gesture.touches[i].identifier);
                    if (curTouchIndex === 0) {
                        first = e.gesture.touches[i];
                    } else if (curTouchIndex === 1) {
                        second = e.gesture.touches[i];
                    } else {
                        Crafty.touches[i] = e.gesture.touches[i].identifier;
                        first = e.gesture.touches[i];
                        if (first) {
                            second = e.gesture.touches[i];
                        }
